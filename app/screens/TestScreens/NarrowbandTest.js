...
function NarrowbandTest({navigation, route}) {
    ...
    const navigateNextPage = () => {
        console.log(toneModule.getNarrowbandResult())
    ...
    const decreaseAmplitude = () => { //click yes
        ...
        if(yes >= 4){
            ...
            const currentDB = toneModule.getDB()
            setYes(-1)
            ...
            toneModule.addNarrowbandResult(average)

            if(toneModule.isAllTested()){ //because need call isAllTested, so cannot nextFreq() first
                toneModule.nextFreq()
                navigateNextPage()
                play = false
            } else {
    ...
    const playSound = () => {
        console.log('threshold:'+toneModule.getThreshold()+' , DB: '+toneModule.getDB()+' ,frequency: '+toneModule.getFrequency()+' ,noise: '+toneModule.getNoiseThreshold())
        setIsPlaying(true)
